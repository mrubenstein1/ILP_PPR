...
#
# And three maps (added when the acquisition schedule from 08 is present):
#   map1_footprint_by_decade   model x climate-scenario, cells shaded by decade
#   map2_foresight_difference  rolling vs myopic acquisition sets (the targeting story)
#   map3_wet_vs_dry            wet vs dry footprints, by model x RCP
#
...
if (DRAW_MAPS && file.exists(SCHEDULE)) {
  ...
  ## MAP 3 — wet vs dry footprints (model rows x RCP cols)
  ...
  save_fig(mp3, "map3_wet_vs_dry", 11, 10.5)
...
message("\nDone. Wrote 2 tables + 6 figures",
        if (drew_maps) " + 3 maps" else "", " to '", OUT_DIR, "/'.")